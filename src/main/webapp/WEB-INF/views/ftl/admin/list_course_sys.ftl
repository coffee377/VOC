...
<div class=" admin-content">
    <div class="admin-biaogelist">
        <div class="listbiaoti am-cf">
            <ul class="am-icon-flag on"> 课程体系列表</ul>

            <dl class="am-icon-home" style="float: right;">当前位置： 课程体系管理 > <a href="#">课程体系列表</a></dl>

            <dl>
                <a type="button" href="edit.html" class="am-btn am-btn-danger am-round am-btn-xs am-icon-plus"> 添加课程体系</a>
            </dl>
            <!--这里打开的是新页面-->
        </div>
        ...
        <form class="am-form am-g">
            <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped am-table-hover">
                ...
                <tbody>
                <#if courseSystemPageBean??>
                    <#list courseSystemPageBean.rows as item>
                    <tr>
                        <td>
                            <input value="${item.csId!}" type="checkbox"/>
                        </td>
                        <td>${item.csId!}</td>
                        ...
                        <td>
                            <div class="am-btn-toolbar">
                                <div class="am-btn-group am-btn-group-xs">
                                    <p value="${item.csId!}" class="update am-btn am-btn-default am-btn-xs am-text-secondary am-round"
                                       data-am-modal="{target: '#my-popups'}" title="修改课程体系">
                                        <span class="am-icon-pencil-square-o"></span>
                                    </p>
                                    <p value="${item.csId!}" class="delete am-btn am-btn-default am-btn-xs am-text-danger am-round"
                                       title="删除课程体系">
        ...
        <#--<hr/>-->
            <div>
            <#if courseSystemPageBean??>
                第 ${courseSystemPageBean.pageNumber} 页 / 共 ${courseSystemPageBean.pages} 页 共 ${courseSystemPageBean.total} 条记录
            </#if>
            </div>
...
<script type="text/javascript">
    $(function () {
        layui.use(["laypage", "layer"], function () {
            ...
            laypage({
                cont: $("#page"),
                pages: <#if courseSystemPageBean??> ${courseSystemPageBean.pages} </#if>,//总页数
                groups: 5, //连续显示分页数
...
//			hash: 'p', //自定义hash值
            ...
            $(".delete").click(function () {
                ...
                layer.confirm("确定要删除此条记录吗？", {icon: 3, title: '提示', offset: '200px'}, function (index) {
                    var url = "${webRoot}/admin/course_sys/delete/" + csId;
                    $.ajax({
                        url: url,   //请求的url地址
                        type: "post",   //请求方式
                        dataType: "json",    //返回格式为
